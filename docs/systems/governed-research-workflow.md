<div class="component-category-page systems-category-page">

<header class="component-category-header">
  <div>
    <p class="map-context">PUBLIC CATEGORY / 02</p>
    <h1>Systems</h1>
    <p>Systems 是面向一类任务的受治理工作流类别：定义进入条件、生命周期、记录关系和人工决定边界。它们在 Framework 的工作区边界内工作，但不替代专门 Skill 的独立合同。</p>
  </div>
  <aside class="component-category-count" aria-label="当前公开目录">
    <strong>CURRENT CATALOGUE</strong>
    <span>1 个公开 System</span>
  </aside>
</header>

<section class="component-category-section" aria-labelledby="systems-catalogue-title">
  <div class="component-category-section-heading">
    <p class="map-kicker">CURRENT PUBLIC COMPONENT</p>
    <h2 id="systems-catalogue-title">当前目录</h2>
  </div>
  <article class="component-entry system-entry">
    <p class="component-entry-kind">SYSTEM</p>
    <div>
      <h3>Governed Research Workflow</h3>
      <p>路由可能的新研究、既有 Study、分析执行、结果权威、手稿、返修、投稿准备和经验治理。</p>
    </div>
    <p class="component-entry-version">v1.10.0</p>
    <a href="https://github.com/chenhaoran2068/governed-research-workflow/releases/tag/v1.10.0">查看 Release <span aria-hidden="true">&rarr;</span></a>
  </article>
</section>

<section class="component-category-section component-facts" aria-labelledby="systems-role-title">
  <div class="component-category-section-heading">
    <p class="map-kicker">CATEGORY ROLE</p>
    <h2 id="systems-role-title">负责什么，不负责什么</h2>
  </div>
  <dl>
    <div>
      <dt>定义</dt>
      <dd>研究任务的进入条件、生命周期、执行与结果记录关系，以及人工决定边界。</dd>
    </div>
    <div>
      <dt>进入时机</dt>
      <dd>可能的新研究，或已明确项目边界的分析、手稿、返修、投稿准备和经验治理任务。</dd>
    </div>
    <div>
      <dt>不决定</dt>
      <dd>伦理或数据访问事实、科学结论、作者资格、投稿、发布，或任何独立 Skill 的自动调用。</dd>
    </div>
  </dl>
</section>

<section class="component-category-section component-category-relationship" aria-labelledby="systems-relationship-title">
  <div class="component-category-section-heading">
    <p class="map-kicker">RELATIONSHIP</p>
    <h2 id="systems-relationship-title">与其他类别的关系</h2>
  </div>
  <p><strong>Frameworks → Systems：</strong>Frameworks 提供工作区和绑定概念；System 在该边界内组织其任务路线、生命周期与正式规则引用。</p>
  <p><strong>Systems ··· Skills：</strong>Skill 不是 System 的下级或默认下一步。仅当某个 System 路线处于特定模式或阶段，用户明确请求并确认所需输入范围时，System 才能受控引用相应的独立 Skill。</p>
</section>

<section class="component-category-section component-sources" aria-labelledby="systems-sources-title">
  <div class="component-category-section-heading">
    <p class="map-kicker">OFFICIAL SOURCES</p>
    <h2 id="systems-sources-title">正式来源</h2>
  </div>
  <ul>
    <li><a href="https://github.com/chenhaoran2068/governed-research-workflow/releases/tag/v1.10.0">v1.10.0 Release</a></li>
    <li><a href="https://github.com/chenhaoran2068/governed-research-workflow/blob/v1.10.0/SKILL.md">System entry</a></li>
    <li><a href="https://github.com/chenhaoran2068/governed-research-workflow/blob/v1.10.0/references/future-study-lifecycle-design-governance-and-analysis-state.md">Future Study lifecycle</a></li>
    <li><a href="https://github.com/chenhaoran2068/governed-research-workflow/blob/v1.10.0/references/research-ethics-preparation-bridge.md">research-ethics bridge</a></li>
  </ul>
</section>

</div>
